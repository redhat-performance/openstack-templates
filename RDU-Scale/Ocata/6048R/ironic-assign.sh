...
# Time-stamp:              <2017-05-09 20:05:41 jfulton> 
# -------------------------------------------------------
if [[ $# -eq 0 ]] ; then
...
def='cpu_hugepages:true,cpu_txt:true,boot_option:local,cpu_aes:true,cpu_vt:true,cpu_hugepages_1g:true'

i=0
for id in $(cat /tmp/n_nodes); do
    ironic node-update $id replace properties/capabilities=profile:$name,$def
    #node="$name-$i"
